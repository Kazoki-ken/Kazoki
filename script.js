...
function login() {
    ...
    fetch('data.json')
        ...
        .then(data => {
            const user = data.users.find(user => (user.username === inputUsername || user.id.toString() === inputUsername) && user.password === inputPassword);
            if (user) {
                isLoggedIn = true;
                currentUser = user;
                ...
                document.getElementById('salomError').style.display = 'block';
...
//             let topRatingsHtml = `<h3>Top Reyting</h3>`;

//             // Foydalanuvchilarni box ichida ko'rsatish
//             sortedUsers.forEach(user => {
//                 const totalCredits = user.credits.reduce((sum, credit) => sum + credit.amount, 0);
//                 topRatingsHtml += `
//                     <div style="border: 2px solid #ccc; padding: 0px; margin: 7px 0; border-radius: 10px; background-color: #f9f9f9;">
